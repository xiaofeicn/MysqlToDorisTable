...
databases = mysql_db.select("select aid from ttk_saas.lb_member where open_statistics=1")
json_arr = []
no_free_table_num = 0
no_free_table_members = []
for database in databases:
    database_name = "ttk_member_" + str(database[0])
    try:
        tables = mysql_db.select(
            "select table_name from {}.lb_free_table where statistics_data_id=0".format(database_name))
        for table in tables:
            tab = table[0]
